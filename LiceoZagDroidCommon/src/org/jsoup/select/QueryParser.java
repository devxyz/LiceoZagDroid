...
class QueryParser {
    ...
    private void findElements() {
        if (tq.matchChomp("#"))
            ...
            has();
        else if (tq.matches(":containsComune("))
            contains(false);
            ...
            throw new Selector.SelectorParseException("Could not parseFromHtml query '%s': unexpected token at '%s'", query, tq.remainder());
    ...
    private void byAttribute() {
        ...
        if (cq.isEmpty()) {
            ...
            if (cq.matchChomp("="))
                ...
                throw new Selector.SelectorParseException("Could not parseFromHtml attribute query '%s': unexpected token at '%s'", query, cq.remainder());
        }
    }
	...
	private void cssNthChild(boolean backwards, boolean ofType) {
		...
		if ("odd".equals(argS)) {
			...
			throw new Selector.SelectorParseException("Could not parseFromHtml nth-index '%s': unexpected format", argS);
		}
		if (ofType)
			if (backwards)
    ...
    // pseudo selector :containsComune(text), containsOwn(text)
    private void contains(boolean own) {
        tq.consume(own ? ":containsOwn" : ":containsComune");
        String searchText = TokenQueue.unescape(tq.chompBalanced('(', ')'));
        Validate.notEmpty(searchText, ":containsComune(text) query must not be empty");
        if (own)
