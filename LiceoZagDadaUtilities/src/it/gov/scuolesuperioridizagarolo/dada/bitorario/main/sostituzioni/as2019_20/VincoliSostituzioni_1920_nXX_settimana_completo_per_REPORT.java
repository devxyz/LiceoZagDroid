 ...
 */
public class VincoliSostituzioni_1920_nXX_settimana_completo_per_REPORT extends AbstractVincoliSostituzioni {
    public static void main(String[] args) throws IOException {
        final VincoliSostituzioni_1920_nXX_settimana_completo_per_REPORT l = new VincoliSostituzioni_1920_nXX_settimana_completo_per_REPORT();
    ...
    protected void preOrarioBeforeAssignment(final BitOrarioGrigliaOrario orarioTotale) {
        for (ClassData x : orarioTotale.getClassi()) {
            System.out.println(x.className);
        }

        for (RoomData x : orarioTotale.getAule()) {
            if (x.flagAulaFittizia())
                continue;
            if (x.maxStudents == 0)
                continue;
            System.out.println(x.roomName+" ("+x.location.description+")");
        }

        if (true) throw new IllegalArgumentException("INTERRUZIONE");
    ...
    protected void postOrarioBeforeFinalCheck(BitOrarioGrigliaOrario orarioTotale, LessonConstraintContainer l) {
        ...
        if (true) throw new IllegalArgumentException("INTERRUZIONE PRIMA DELLA CREAZIONE DEI FILES");
    }
    ...
    public VincoliSostituzioni_1920_nXX_settimana_completo_per_REPORT invoke(final BitOrarioGrigliaOrario orarioTotale, final LessonConstraintContainer l) {
